...
#define MAX9296A_PINCTRL_NAME		MAX9296A_NAME "-pinctrl"
...
#define MAX9296A_GROUP_PINS(name, ...) static const unsigned int name ## _pins[] = { __VA_ARGS__ }
...
static const struct pingroup max9296a_ctrl_groups[] = {
	...
	MAX9296A_GROUP(mfp8),
	MAX9296A_GROUP(mfp9),
	MAX9296A_GROUP(mfp10),
	MAX9296A_GROUP(mfp11),
...
MAX9296A_FUNC_GROUPS(gpio, "mfp0", "mfp1", "mfp2", "mfp3", "mfp4", "mfp5", "mfp6",
...
MAX9296A_FUNC_GROUPS(errb, "mfp4");
...
static int max9296a_ctrl_get_group_pins(struct pinctrl_dev *pctldev, unsigned selector,
			    ...
			    unsigned group)
{
	...
	switch (selector) {
	case max9296a_func_gpio:
		if (group == 5 || group == 6) {
			ret = regmap_update_bits(priv->regmap, 0x003, 0x10, 0x00);
		...
		if (group == 1) {
			...
			ret = regmap_update_bits(priv->regmap, 0x005, 0x40, 0x00);
		}
		break;
	...
	case max9296a_func_lock:
		...
		break;
	case max9296a_func_errb:
		ret = regmap_update_bits(priv->regmap, 0x005, 0x40, 0x40);
		break;
	}
	return ret;
}
